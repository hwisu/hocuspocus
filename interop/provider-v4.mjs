...
const ddqClientMixedRoot = Buffer.from(
	// Exact ddq-client AnswerDocAttrs + @tiptap/y-tiptap 3.0.8 + Yjs
	// 13.6.32 fixture. SHA-256:
...
const expectedDdqClientXml =
	'<paragraph index="0" node_ids="source-node-1" prompt="Explain the control">클라이언트 호환성 𠮷</paragraph>';
...
const encodedStateVector = (document) =>
	Buffer.from(Y.encodeStateVector(document)).toString("base64");
...
const createClient = () => {
	...
	// Tiptap Collaboration.configure materializes its named XmlFragment before
	// the provider applies remote updates.
	document.getXmlFragment("9253");
	const stateless = [];
	let resolveSynced;
	let rejectSynced;
...
try {
	...
	await waitFor(
		() =>
			first.stateless.includes("token:oracle-token") &&
			second.stateless.includes("token:oracle-token"),
		"v4 token refresh",
	);
	...
	await waitFor(
		() => !first.provider.hasUnsyncedChanges,
		"answer document acknowledgement",
	);
	await waitFor(
		() =>
			canonicalJson(answerDocumentState(second.document)) ===
				canonicalJson(expectedAnswerDocument) &&
			encodedStateVector(second.document) ===
				encodedStateVector(first.document),
		"nested map and Tiptap XML answer document",
	);
	expectedStateVector = encodedStateVector(first.document);

	Y.applyUpdate(first.document, ddqClientMixedRoot);
	await waitFor(
		() => !first.provider.hasUnsyncedChanges,
		"ddq-client mixed-root acknowledgement",
	);
	await waitFor(
		() =>
			second.document.getXmlFragment("9253").toString() ===
			expectedDdqClientXml,
		"ddq-client legacy Array to Tiptap XmlFragment update",
	);
	expectedStateVector = encodedStateVector(first.document);
	...
	await waitFor(
		async () =>
			(await fetch(`http://127.0.0.1:${port}/documents-count`).then(
	...
	await waitFor(
		() =>
			canonicalJson(answerDocumentState(reconnected.document)) ===
				canonicalJson(expectedAnswerDocument) &&
	...
	await waitFor(
		() =>
			reconnected.document.getXmlFragment("9253").toString() ===
			expectedDdqClientXml,
		"persisted ddq-client mixed-root document after reconnect",
	);
