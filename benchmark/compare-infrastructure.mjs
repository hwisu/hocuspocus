...
const output = path.resolve(
	...
	argumentsByName.get("output") ??
		"hocuspocus-benchmark/build/reports/ab/infrastructure-latest.json",
);
const jvmExecutable =
	process.env.HOCUSPOCUS_JVM_BENCHMARK_EXECUTABLE ??
	path.join(
		repoRoot,
		"hocuspocus-benchmark/build/install/hocuspocus-benchmark/bin/hocuspocus-benchmark",
	);
const jvmOptions =
...
const gate = compatibilityOnly
	? {
			...
			policy: {
				compatibility:
					"all cross-runtime SQLite, Redis, awareness-add, Node-to-JVM awareness-removal, stateless, reconnect, and store-lock contracts pass",
			},
			failures: [],
		}
...
async function verifyMixedRedisDirection(sourceServer, recipientServer, name) {
	...
	try {
		...
		// The published Node 4.7.0 server removes owned awareness on disconnect.
		...
		try {
			awareness.provider.awareness.setLocalStateField(
				"mixedRuntime",
				awarenessMarker,
			);
			await waitUntil(
			...
			awareness.provider.destroy();
			awareness.document.destroy();
		}
		if (recipientServer.target === "jvm") {
			try {
				await waitUntil(
					() =>
						![...recipient.provider.awareness.getStates().values()].some(
...
async function startTarget(target, mode, extraEnvironment) {
	...
	const commandArguments =
		target === "node"
			? [path.join(repoRoot, "benchmark/upstream-server.mjs")]
			: [];
