...
const jvmDirectory = path.join(repository, "jvm");
const lockfiles = fs.readdirSync(jvmDirectory)
  .map((name) => path.join(jvmDirectory, name, "gradle.lockfile"))
  .filter(fs.existsSync)
  .sort();

assert.ok(lockfiles.length > 0, "no JVM dependency lockfiles were found");
