...
separate processes, alternates their order, and drives both with the same pinned
...
Run only the cross-runtime migration contracts:
...
The official Node 4.7.0 server does not consistently apply an awareness
...
Node WebSocket routes before handing their documents to JVM instances. CI
enforces this compatibility-only phase independently from platform-sensitive
infrastructure performance ratios.
...
OpenJDK 21.0.11, the pinned official Provider v4, and the local YKS
...
The following Apple M4 baseline passed its executable gate before the
repository switched from vendored Node sources to pinned official 4.4.0 npm
packages:
...
Treat those ratios as a historical machine baseline, not a universal pass.
The JVM-only repository keeps cross-runtime compatibility as the required CI
gate; performance ratios are rerun explicitly on each target platform.
